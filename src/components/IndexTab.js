    ...
    if (this.props.context.isDevelop === true) {
      ...
      posts = filter(this.props.context.posts, item => {
        return (
          ...
          moment(item.node.frontmatter.date, 'MMMM DD, YYYY').unix() <=
            moment().unix()
        )
      })
    }
      ...
      <React.Fragment>
        ...
        <ContentContainer>
          {this.state.tabIndex === 0 ? (
            this.props.context.posts.length === 0 ? (
          ...
          {this.state.tabIndex === 0 ? (
            <MoreButtonWrapper>
              {!this.props.context.isLast ? (
                <Link to={'/' + (this.props.context.page + 1)}>
                  <PrimaryButton float="left" label="Older Posts" />
                </Link>
              ) : null}
              {!this.props.context.isFirst && this.props.context.page !== 2 ? (
                <Link to={this.props.context.page - 1}>
                  <PrimaryButton float="right" label="Newer Posts" />
                </Link>
              ) : null}
              {this.props.context.page === 2 ? (
                <Link to={'/'}>
                  <PrimaryButton float="right" label="Newer Posts" />
                </Link>
              ) : null}
          ...
          {this.state.tabIndex === 1 ? (
            this.props.context.categories.length === 0 ? (
